...
> ⚠ **最高規則（2026-08-21 使用者指定）：筆記生成／更新（情境 1、2）完成後，一律停在 Google 文件這一步，不自動部署。**
> 流程 F（Speak Up 出考題）、流程 F2（DAYTALK）、流程 G（Firebase 部署）、流程 H（B2 Read 課本）都只在使用者**明確要求**時才執行
> （例如說「同步 Speak Up／出考題／部署／更新課本／同步筆記」）。完成筆記後在回報中提醒一句「Speak Up 與 B2 Read 尚未同步」即可。
...
- 單字表格固定 5 欄：單字｜音標｜詞性｜**例句**｜**中文**（例句格內＝英文句（該課單字粗體）＋換行＋中文翻譯）
  ...
  - **通則（2026-08-29 使用者指定，取代 2026-08-21「例句放最右」的舊規則）：任何表格只要同時有「例句」欄與「中文」欄，一律「例句」在前、「中文」在後 —— 中文意思欄放最右**
    ...
    - ⚠ **片語表自 2026-08-25 起改成索引表**：`片語｜中文｜詳見`（**不放例句**），例句只寫在對應主題章節。
      這張表沒有「例句」欄，所以上面的欄序通則不適用。理由與完整去重規則見下方「⭐ 內容去重原則」。
...
5. **音標全部是美式**（見「共用：教材內容規範」的音標規則）：沒有 `ɒ`／`ɜː`／`əʊ`／`ɛ`，而且每個單字「拼字裡的 r」都在音標裡有對應的 `r`／`ɚ`／`ɝ`（2026-09-21 新增；之前就是漏了捲舌，全站 1,300 多筆音標才要重掃一次）

確認無誤後才進行渲染貼上；若發現問題，先修正內容再產出，**不要**把有疑慮的文法解說直接貼到 Google 文件。

## ⭐ 內容去重原則：一句例句只出現一次（2026-08-25 確立，使用者回饋）

> 這一節曾在 2026-08-28 的 rebase 事故中連同 `edf92b7` 一起落單，2026-08-29 補回。

2026-08-25 使用者回報「裡面內容有很多一直重複」。原因是每個章節各自附例句，同一句話在
作業複習、單字表、片語表、主題章節、正確句總表、快速總結裡出現 5～6 次。
**產出前一定要跑這一輪去重檢查**，格式（橘色 bar／黃色 callout／橘色表頭表格）維持不變，只調整內容分布。

### 硬性規則

1. **每個句子只有一個「家」**：一句英文例句在整份文件只完整出現一次，放在它最該在的章節。
   - 作業原句／訂正句 → 只放 **I. 作業複習**
   - 主題句型例句 → 只放該主題章節（III～VII）
   - 其他章節要提到時，只寫規則、對比或中文說明，**不要把整句再貼一次**
2. **不要做「正確句總表」這種章節**：內容 100% 是別章的複製。作業複習本身就是總表。
   （2026-08-25 已從 20260825 v2 移除。）
3. **片語表（II-B）改成索引表**：欄位為 `片語｜中文｜詳見`，
   `詳見` 填該片語詳細講解的章節代號（如 `IV-A`、`VII-B`）。
   例句只寫在對應章節，片語表不重複貼例句。
   表格上方加一行 callout 說明：「本表只列片語＋中文，方便快速查找；詳細用法、例句與比較請看後面各章。」
4. **單字表（II-A）用短例句**：若該字的完整例句已在作業複習出現，單字表改寫**精簡版短句**
   （例：作業句 `The cloud service was down, which caused the document to become corrupted.`
   → 單字表 corrupted 寫 `The document became corrupted.`），不要整句照抄。
5. **同一個點只列一次**：
   - 時差章節「句型列」與「時間換算列」合併成同一列（一地區一列），不要拆成兩列各講一次
   - 品味章節不要列 9 列 `have the same taste in ○○`；領域清單併進 callout 一行帶過，
     表格只留**語氣差異**（相似／相同／不同／完全不同）各一句
   - 主題章節內的 A/B/C 小節之間不要互相重貼例句
6. **快速總結只寫規則、不貼完整例句**：用「A is [n] hours **ahead of** B（一定加 of）」這種
   規則式寫法，不要把整句範例再抄一次。
7. **填空練習合併重複考點**：同一個文法點不要出兩題；一題可以挖兩個空
   （例：`Taiwan is one hour ___ ___ Vietnam, so Vietnam is one hour ___ Taiwan.` → ahead of / behind）。

### 產出前檢查方式

把所有英文例句抓出來比對，**同一句出現 2 次以上就要砍到剩 1 次**（作業複習優先保留）。
去重後章節編號要重新連號（移除章節後，後面的 VIII / IX / X 要往前遞補）。

> ⚠ **同步到 B2 Read 課本（流程 H）時同樣適用**：`phrases[]` 不要每條都掛 `ex`／`exCn`，
> 那些句子多半已經在 `hw` / `grammar` / `extra` 出現過。片語只留 `p` + `cn`，
> 需要指路就把章節代號寫進 `cn`（例：`延後、拖延（詳見 IV-B）`）。
> 渲染器已支援省略 `ex`（2026-08-29 起 `p.ex` 為空就不畫例句列）。
> `bk20260825` 是規則確立當天做的，`phrases` 還留著例句，屬既有資料，不回頭改。
...
- **紅字／藍字**：`❌ 原句` 用 `color:#FF0000`、`✅ 訂正` 用 `color:#0000FF`
- **表格只有標題列粗體**：`<th>` 粗體、`<td>` 明確寫 `font-weight:normal`
- `<h1>/<h3>` 階層、⭐ ⚠ ❌ ✅ 等符號

**❌ HTML 轉檔後一定會掉、必須用 Apps Script 補的五件事（2026-08-25 使用者驗收確認；
2026-08-28 rebase 事故中落單，2026-08-29 補回）：**

| 項目 | 正確值 | 轉檔結果 | Apps Script 補法 |
|------|--------|----------|------------------|
| 段落行距 | **1.5** | 掉回 1.0／1.15 | `p.setLineSpacing(1.5)` 走訪所有段落（含表格儲存格內段落） |
| 側邊黃底欄寬 | **0.4 公分** | 被拉寬或壓扁 | `t.setColumnWidth(0, 11.34)`（11.34pt ≒ 0.4cm），對每張 callout 表格套用 |
| 紅色 | **`#FF0000`** | CSS 有效但舊檔常是 `#C0392B` | 走訪文字，`getForegroundColor()` 是舊紅就 `setForegroundColor("#FF0000")` |
| 藍色 | **`#0000FF`** | 同上，舊檔常是 `#1155CC` | 同上，改成 `#0000FF` |
| 章節分頁 | 每個橘色 bar 各自起新頁 | `page-break-before:always` **不會**轉成分頁符號 | 由後往前 `body.insertParagraph(i, "").appendPageBreak()`，第一個 bar 略過 |

⚠ 這五項不要指望 HTML CSS——`line-height`、`page-break-before`、`<colgroup>` 在 Drive 轉檔時都會被丟掉。
**HTML 上傳只負責內容與底色，版面一律靠轉檔後的 Apps Script 修正腳本收尾。**
可重複使用的修正腳本：`20260825-時差表達與默契用語/fix-format.gs`（只改格式、不動內容，換 `DOC_ID` 即可套用到任何一份筆記）。

**⚠ 比照 20260811 的實際樣式規格（2026-08-13 使用者驗收通過的版本）：**
- 字體一律 `'Google Sans'`；H1 20pt／橘色 bar 15pt／小標題 14pt／內文與表格 13pt
- callout 用**兩欄表格**：第一欄背景 `#FFD54F`（黃色粗直條），第二欄 `#FFFDE7`。
  ⚠ 不要用 `border-left`，那條會太細，跟 20260811 不一樣
  ⚠ **黃色直條欄寬一律 0.4 公分**（轉檔後用 Apps Script `table.setColumnWidth(0, 11.34)`，11.34pt≈0.4cm；2026-08-19 使用者指定）
- callout 內文字顏色：標籤（原句／訂正／中文／結構／文法解說）**粗體黑字**；
  **訂正句本身是黑字，只有被修正的那幾個字用紅色斜體**（`#FF0000` + italic）——整句藍色是錯的；
  **`結構：` 後面的句型才用藍色粗體**（`#0000FF`）
...
7. ⚠ **筆記生成後不要自動部署**（2026-08-21 使用者指定）：**不要**自動接著執行流程 F（出考題／Speak Up）、流程 G（Firebase 部署）或流程 H（B2 Read 課本）；等使用者明確要求「同步／出考題／部署／更新課本」時才做
8. 完成後回報 Google 文件連結 + 教材摘要（幾個單字、幾個句型等），並提醒使用者：Speak Up 與 B2 Read 課本尚未同步，需要時再說一聲
...
出考題功能。**觸發時機**（2026-08-21 改為只有使用者明確要求才執行，建立筆記後**不再自動接著執行**）：
- 使用者說「出考題 / 生成考題 / 更新測驗 / 幫我出題 / 同步 Speak Up」，或針對某一天筆記要求出題
...
DocumentApp helper（每張卡是一個 2 欄無框表格：左黃條 10pt + 內容欄；用 `editAsText().setBold(0, label.length-1, true)` 粗體化標籤，用 `setItalic` + `setForegroundColor(idx, idx+len-1, "#FF0000")` 標紅斜體被改的字）：

```javascript
var STRIP="#FFD54F", CALLOUT="#FFFDE7", REDW="#FF0000", F="Google Sans";
...
**觸發時機**：**只在使用者明確要求時執行**（說「更新課本／同步 B2 Read／同步課本」等），把該課筆記同步到 B2 Read 的「📕 課本」分頁。（2026-08-21 使用者指定：建立或更新課堂筆記 Google Doc 之後**不再自動接著執行**本流程。）
...
- **動詞三態寫法（2026-08-24 確立）**：extraVocab / summary 裡的不規則動詞三態一律寫成「`原形 – 過去式 – 過去分詞`」（用 en dash `–` 或 hyphen 分隔，如 `lend – lent – lent`）。渲染器（index.html 的 `bkForms`）會自動把原形加粗、變化形上橘色，資料端不要自己加 HTML 標籤。
- **重音音節寫法（2026-08-24 確立）**：要標示單字重音時，在 extraVocab / summary 的英文欄用「音節連字號＋重音音節全大寫」寫法（如 `spe-ci-fi-CA-tion`）。渲染器會自動把大寫音節變成紅色加粗底線；中文備註就不用再寫「重音在 ca」這種文字說明。
- **故事插圖箭頭**：五圓插圖產生器 `art()` 已自動在圓圈之間畫進度箭頭（➔），新課只要照第 3 步加 `S[lessonId]` 即可，不用另外處理；若手寫整段 `art:` SVG（非用產生器），記得在 `</svg>` 前補上同樣的箭頭群組（x=147/301/455/609、y=100，`h38 l-10 -8 m10 8 l-10 8`）。
