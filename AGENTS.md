...
- **文章與實景的單字例句不可照抄本文**：閱讀／每日／A2 的 `target[].ex` 與實景 `vocab[].ex`
  要原創、換情境（不沿用本文的人物、地點、事件），不能跟本文、quotes、exs 相同或只是改寫；
  每句要有繁中 `exCn`。`b2lab/DAILY_TASK.md` 第 5 步驗證會擋「與本文連續 4 字相同」。
- **文法頁「重複文法，不重複句子」**：同一句完整例句整頁最多 2 次。Step 1 放主例句，
  情境卡／逐步理解／比較區／陷阱／練習題換不同情境（生活、工作、旅行、QA、家庭）。
  細則見 `b2lab/DAILY_TASK.md` 第 3 節，每日生成的驗證會自動擋。
- **實景英語（`ST.rw`，`rw*` 系列函式）**：
